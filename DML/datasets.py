...
def give_InShop_datasets(opt):
    ...
    data_info = np.array(pd.read_table(opt.source_path+'/Eval/list_eval_partition.txt', header=1, sep='\s+'))[1:,:]
    ...
    #Generate conversions(color verson)
    ...
    # max_runk = 120
    # colors = {}
    # with open(opt.source_path+'/Eval/list_color_cloth.txt') as f:
    #     f.readline()
    #     f.readline()
    #     for line in f:
    #         x = line.rstrip().split()
    #         if x[1] not in colors.keys():
    #             colors[x[1]] = 1
    #         else:
    #             colors[x[1]] += 1
    ...
    # use_all_colors = []
    # for i in range(max_runk):
    #     use_all_colors.append(colors_sorted[i][0])
    ...
    # new_query_index = []
    # new_gallery_index = []
    # cloth_datas = [train, query, gallery]
    # for i, a in enumerate(cloth_datas):
    #     for j, c_id in enumerate(a[:,0]):
    #         if id_color[c_id] in use_all_colors:
    #             if i == 0:
    #                 new_train_index.append(j)
    #             if i == 1:
    #                 new_query_index.append(j)
    #             if i == 2:
    #                 new_gallery_index.append(j)
                    
    # train = train[new_train_index]
    # query = query[new_query_index]
    # gallery = gallery[new_gallery_index]
    # ---------------------------------------------------------------------------------------------------
    ...
    # for i, a in enumerate(cloth_datas):
    #     for j, c_id in enumerate(a[:,0]):
    #         if "full.jpg" not in c_id.split("_"):
    #             if i == 0:
    #                 new_train_index.append(j)
    #             if i == 1:
    #                 new_query_index.append(j)
    ...
    for i, a in enumerate(cloth_datas):
        for j, c_id in enumerate(a[:,0]):
            if i == 0:
                if id_color[c_id] not in train_colors:
                    train_colors.append(id_color[c_id])
                    train[j,1] = train_colors.index(id_color[c_id])
                else:
                    train[j,1] = train_colors.index(id_color[c_id])
            elif i == 1:
                if id_color[c_id] not in query_and_gallery_colors:
                    query_and_gallery_colors.append(id_color[c_id])
                    query[j,1] = query_and_gallery_colors.index(id_color[c_id])
                else:
                    query[j,1] = query_and_gallery_colors.index(id_color[c_id])
            else:
                if id_color[c_id] not in query_and_gallery_colors:
                    query_and_gallery_colors.append(id_color[c_id])
                    gallery[j,1] = query_and_gallery_colors.index(id_color[c_id])
                else:
                    gallery[j,1] = query_and_gallery_colors.index(id_color[c_id])
